...
from langchain_experimental.agents.agent_toolkits import create_pandas_dataframe_agent  #创建df agent执行器的函数 

#通过让agent返回字典，根据字典的不同键来判断输出文字，表格，还是图
PROMPT_TEMPLATE = """
...
def dataframe_agent(openai_api_key, df, query):
    ...
    agent = create_pandas_dataframe_agent(llm=model,
                                          df=df,
                                          agent_executor_kwargs={"handle_parsing_errors": True},  #尽可能让模型自行消化错误，避免程序终止
                                          verbose=True)    #在终端看到执行过程，了解模型是怎么思考的
    prompt = PROMPT_TEMPLATE + query
    response = agent.invoke({"input": prompt})
    response_dict = json.loads(response["output"])  #将字符串解析为字典，方便前端使用
